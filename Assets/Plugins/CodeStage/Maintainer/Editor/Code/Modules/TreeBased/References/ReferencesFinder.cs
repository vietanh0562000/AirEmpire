...
namespace CodeStage.Maintainer.References
{
	...
	public class ReferencesFinder
	{
		...
		private static bool LookForReferences(FilterItem[] selectedAssets, List<ReferencesTreeElement> results)
		{
			...
			if (!canceled)
			{
				...
				for (var i = 0; i < count; i++)
				{
					...
					if (CSEditorTools.IsValueMatchesAnyFilter(assetInfo.AssetPath, MaintainerSettings.References.pathIgnoresFilters)) continue;
		...
		private static ReferencesTreeElement BuildTreeBranchRecursive(AssetInfo assetInfo, int depth, int id, List<ReferencesTreeElement> results)
		{
			...
			{
				id = id + results.Count,
				name = CSPathTools.NicifyAssetPath(assetInfo.AssetPath, assetInfo.Kind),
				assetPath = assetPath,
			...
			if (assetInfo.referencedAtInfoList.Count > 0)
			{
				foreach (var referencedAtInfo in assetInfo.referencedAtInfoList)
				{
					...
					if (referencedAtType == CSReflectionTools.gameObjectType || referencedAtType == CSReflectionTools.sceneAssetType || referencedAtType == CSReflectionTools.monoBehaviourType || referencedAtType == CSReflectionTools.monoScriptType)
					{
						...
						else
						{
							...
							if (tc == null)
							{
								...
								if (assetType == CSReflectionTools.gameObjectType ||
									assetType == CSReflectionTools.fontType ||
									assetType == CSReflectionTools.texture2DType ||
#if !UNITY_2018_1_OR_NEWER
									assetType == CSReflectionTools.substanceArchiveType ||
#endif
									assetType == CSReflectionTools.defaultAssetType && assetPath.EndsWith(".dll") ||
		...
		private static bool FillReferenceEntries()
		{
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (item.asset.Type == CSReflectionTools.gameObjectType)
				{
					ProcessPrefab(item);
				}
				else if (item.asset.Type == CSReflectionTools.sceneAssetType)
				{
					ProcessScene(item);
				}
				else if (item.asset.Type == CSReflectionTools.monoBehaviourType || item.asset.Type == CSReflectionTools.monoScriptType)
				{
		...
		private static void ProcessSceneSettings(AssetConjunctionInfo conjunctionInfo)
		{
			...
			foreach (var conjunction in conjunctionInfo.conjunctions)
			{
				if (conjunction.referencedObjectsCandidates.Length == 1)
				{
					...
					{
						lightmapSettings = lightmapSettings ? lightmapSettings : CSEditorTools.GetLightmapSettings();
				...
				else
				{
					foreach (var candidate in conjunction.referencedObjectsCandidates)
					{
						...
						if (texture2D != null)
						{
							renderSettings = renderSettings ? renderSettings : CSEditorTools.GetRenderSettings();
...
#else
					var prefabParent = PrefabUtility.GetPrefabParent(inspectedUnityObject);
#endif
