...
namespace CodeStage.Maintainer.Cleaner
{
	...
	public class AssetRecord : CleanerRecord, IShowableRecord
	{
		...
		public void Show()
		{
			if (!CSSelectionTools.RevealAndSelect(assetDatabasePath, null, -1, -1, null))
			{
		...
		internal override bool MatchesFilter(FilterItem newFilter)
		{
			...
			switch (newFilter.kind)
			{
				...
				case FilterKind.Extension:
					return !string.IsNullOrEmpty(assetDatabasePath) && CSEditorTools.IsValueMatchesAnyFilterOfKind(assetDatabasePath, filters, newFilter.kind);
				case FilterKind.Type:
					return assetType != null && CSEditorTools.IsValueMatchesAnyFilterOfKind(assetType.Name, filters, newFilter.kind);
