...
namespace CodeStage.Maintainer.Issues
{
	...
	public class ObjectIssueRecord : IssueRecord, IShowableRecord
	{
		...
		public void Show()
		{
			if (!CSSelectionTools.RevealAndSelect(path, transformPath, objectId, componentIndex, propertyPath))
			{
		...
		internal override bool MatchesFilter(FilterItem newFilter)
		{
			...
			switch (newFilter.kind)
			{
				...
				case FilterKind.Extension:
					return !string.IsNullOrEmpty(path) && CSEditorTools.IsValueMatchesAnyFilterOfKind(path, filters, newFilter.kind);
				case FilterKind.Type:
				{
					return !string.IsNullOrEmpty(componentName) && CSEditorTools.IsValueMatchesAnyFilterOfKind(componentName, filters, newFilter.kind);
				}
		...
		protected override void ConstructBody(StringBuilder text)
		{
			...
			var nicePath = path == "" ? "Untitled (current scene)" : CSPathTools.NicifyAssetPath(path, true);
