...
namespace CodeStage.Maintainer.Tools
{
	...
	public class CSSceneTools
	{
		...
		{
			if (lastOpenSceneResult == null) return;

			if (!string.IsNullOrEmpty(nextScenePath) && nextScenePath == lastOpenSceneResult.scenePath) return;

			if ((lastOpenSceneResult.sceneWasLoaded || forceClose) && SceneManager.loadedSceneCount > 1)
			{
