					...
					if (new Version(settings.version) < new Version("1.5.1"))
					{
						    ...
						    settings.projectCleanerSettings.pathIgnoresFilters.Length > 0)
						{
							...
							for (var i = settings.projectCleanerSettings.pathIgnoresFilters.Length - 1; i >= 0; i--)
							{
								...
								if (CSEditorTools.IsValueMatchesAnyFilterOfKind(pathIgnoresFilter.value, mandatoryFilters, pathIgnoresFilter.kind))
								{
									isMandatory = true;
								}
								...
								if (CSEditorTools.IsValueMatchesAnyFilterOfKind(pathIgnoresFilter.value, defaultFilters, pathIgnoresFilter.kind))
								{
