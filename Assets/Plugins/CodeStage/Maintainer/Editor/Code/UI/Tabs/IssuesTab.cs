...
namespace CodeStage.Maintainer.UI
{
	...
	using RecordType = Issues.RecordType;
	...
	internal class IssuesTab : RecordsTab<IssueRecord>
	{
		...
		protected override void DrawSettingsBody()
		{
			...
			{
				GUILayout.Space(5);
				GUILayout.Label("<b><size=12>Issues to look for</size></b>", UIHelpers.richLabel);
				UIHelpers.Separator();
				GUILayout.Space(5);
				...
				if (MaintainerSettings.Issues.gameObjectsFoldout)
				{
					...
					{
						MaintainerSettings.Issues.emptyArrayItems = EditorGUILayout.ToggleLeft(new GUIContent("Empty array items", "Look for any not assigned array items."), MaintainerSettings.Issues.emptyArrayItems);
			...
			if (!string.IsNullOrEmpty(record.path))
			{
				...
				if (dir.Name != "Assets")
				{
					menu.AddItem(new GUIContent("Ignore/Parent Folder"), false, () =>
					{
						...
						if (!CSEditorTools.IsValueMatchesAnyFilter(dirPath, MaintainerSettings.Issues.pathIgnoresFilters))
						{
