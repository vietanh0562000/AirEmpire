...
namespace CodeStage.Maintainer.UI
{
	...
	internal class ReferencesTreeView<T> : MaintainerTreeView<T> where T : ReferencesTreeElement
	{
		...
		protected override DragAndDropVisualMode HandleDragAndDrop(DragAndDropArgs args)
		{
			...
			var atLeastOneMonoBehaviourDropped = false;
			for (var i = 0; i < objectReferences.Length; i++)
			{
				...
				atLeastOneMonoBehaviourDropped = true;
				objectReferences[i] = monoScript;
			}

			if (!atLeastOneMonoBehaviourDropped && (paths == null || paths.Length == 0))
			{
				return DragAndDropVisualMode.Rejected;
			}
			...
			{
				var assetsPaths = new List<string>(objectReferences.Length);
		...
		private static void ShowItem(ReferencesTreeViewItem<T> item, ReferencingEntryData referencingEntry = null)
		{
			...
			if (referencingEntry != null)
			{
				...
				switch (referencingEntry.location)
				{
					case Location.GameObject:
						...
						if (!CSSelectionTools.RevealAndSelect(assetPath, referencingEntry.transformPath, referencingEntry.objectId, referencingEntry.componentId, referencingEntry.propertyName))
						{
							MaintainerWindow.ShowNotification("Can't show it properly");
						}
						break;
			...
			else
			{
				...
				{
					if (!CSSelectionTools.RevealAndSelect(assetPath, string.Empty, -1, -1, string.Empty))
					{
						MaintainerWindow.ShowNotification("Can't show it properly");
					}
				}
